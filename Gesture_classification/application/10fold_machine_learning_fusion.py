...
from framework.data_generator import DataGeneratorgesture
...
def get_ml_features(audio_feature, tactile_feature):
    audio_feature = audio_feature.reshape((audio_feature.shape[0], -1))
    tactile_feature = tactile_feature.reshape((tactile_feature.shape[0], -1))

    return np.concatenate((audio_feature, tactile_feature), axis=1)
...
def main(argv):
    ...
    for fold_index in range(10):
        ...
        generator = DataGeneratorgesture(renormal=True, clip_length=1000, batch_size=32,
                                         test_size=0.165, val_size=0.1, seed=42,
                                         fold_index=fold_index, folds_num=10)

        train_audio = generator.transform(generator.gesture_training_a, generator.mean_audio, generator.std_audio)
        train_tactile = generator.transform(generator.gesture_training_t, generator.mean_tactile, generator.std_tactile)
        val_audio = generator.transform(generator.Emotion_validation_a, generator.mean_audio, generator.std_audio)
        val_tactile = generator.transform(generator.Emotion_validation_t, generator.mean_tactile, generator.std_tactile)

        X_train = get_ml_features(train_audio, train_tactile)
        X_val = get_ml_features(val_audio, val_tactile)
        y_train = generator.gesture_training_label
        y_val = generator.Emotion_validation_label
    ...
    generator = DataGeneratorgesture(renormal=True, clip_length=1000, batch_size=32,
                                     test_size=0.165, val_size=0.1, seed=42,
                                     use_all_training_ids=True)

    train_audio = generator.transform(generator.gesture_training_a, generator.mean_audio, generator.std_audio)
    train_tactile = generator.transform(generator.gesture_training_t, generator.mean_tactile, generator.std_tactile)
    test_audio = generator.transform(generator.Emotion_test_a, generator.mean_audio, generator.std_audio)
    test_tactile = generator.transform(generator.Emotion_test_t, generator.mean_tactile, generator.std_tactile)

    X_train_total = get_ml_features(train_audio, train_tactile)
    X_test = get_ml_features(test_audio, test_tactile)
    y_train_total = generator.gesture_training_label
    y_test = generator.Emotion_test_label
