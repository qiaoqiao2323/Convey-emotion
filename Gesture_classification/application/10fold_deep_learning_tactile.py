...
def main(argv):
    ...
    sys_name = 'sys_' + model_name + '_tactile'
    ...
    for fold_index in range(fold_num):
        ...
        if not os.path.exists(log_path):
            ...
            model = SingleModalModel(using_model(len(config.Gesture_list_name)), 'tactile')
            model.to(device)
            ...
            generator = DataGeneratorgesture(renormal=True, clip_length=clip_length, batch_size=batch_size,
                                             ...
                                             fold_index=fold_index, folds_num=fold_num, modality='tactile')

            fold_result = training_process_10fold(generator, model, models_dir, epochs, batch_size,
